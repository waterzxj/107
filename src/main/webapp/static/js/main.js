...
var room107 = {
    ...
    bindLogin : function(callback){
        ...
        $(document).bind('room107.login', function(e, options){
            callback(options);
        });
    },
...
function login(user) {
    ...
    */
    ...
    // user
    room107.username = user.username;
    room107.userType = user.type;
    room107.regRole = user.regRole;
    room107.email = user.email;
    
    // hook for those operations would run before 'refreshLoginStatus()'
    if(room107.execBeforeLoginRefresh()){
        return;
    }
    //refresh login status on header
    refreshLoginStatus();

    $(document).trigger('room107.login', {'user' : user});
    // redirect to search from index and login page
    var path = window.location.pathname;
    if (path.lastIndexOf('/') === (path.length - 1)) {
        path = path.substring(0, path.length - 1);
    }    
    //TODO remove oauth latter
    if (path === '' || path === '/index' || path === '/user/login' || path === '/house/search' || path === '/user/login/oauth') {
        if (room107.isLandlord() || room107.isRegLandlord()) {
            window.location = '/house/manage';
        } else {
            window.location = '/house/search';
        }
    } else if (path == '/house/manage') {
        if (room107.isLandlord()) {
            window.location = '/house/manage';
        }
    }
    // totoro
    $('#antispam1').attr('src', '/antispam/history/');
}
...
function refreshLoginStatus() {
	if (room107.auth == 0) {
		...
		$(".unverifiedTag").hide();
		$(".verifiedTag").hide();
	} else if (room107.auth == 1) {
		$(".unloginTag").hide();
		$(".unverifiedTag").show();
		$(".verifiedTag").show();
		$("#logout").attr("title", room107.email);
	} else {
		$(".unloginTag").hide();
		$(".unverifiedTag").hide();
		$(".verifiedTag").show();
...
var Lightbox = {
    ...
    open : function(url, urlParam, callback, option) {
        ...
        // init
        option = option || {};
        var c = $('#lightboxContent'), loading = $('#lightboxBg .loading').hide();
        $('#lightboxClose').hide();
        loading.hide();
        $('#lightbox').show();
...
var Input = {
    ...
    validate : function(input, force) {
        ...
        if (v.indexOf('int') == 0) {
            ...
            if (inBound(input.val().length, v.substring(6)) != 0)
                result = false;
